...
-- Contact person within customer organizations
...
-- Customer service subscriptions (ERP master data)
CREATE TABLE customer_services (
    ...
    -- CRM Tracking
    sales_opportunity_id UUID, -- Links to opportunity that generated this
...
CREATE TABLE erp_sync_log (
    ...
    entity_id VARCHAR(50) NOT NULL, -- ERP ID synced
...
GROUP BY c.customer_id, c.company_name;
    COUNT(st.ticket_id) as open_tickets,
    DATEDIFF(CURDATE(), MAX(i.interaction_date)) as days_since_last_interaction,
    CASE 
        WHEN AVG(cs.churn_risk_score) > 0.7 THEN 'At Risk'
        WHEN AVG(cs.churn_risk_score) > 0.4 THEN 'Medium Risk'
        ELSE 'Healthy'
